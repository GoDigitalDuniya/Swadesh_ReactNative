...
export default function Inward({ searchQuery }: InwardScreenProps) {
  ...
  // Data fetching logic
  const fetchInwards = useCallback(async (query: string) => {
    if (!refreshing) {
        setLoading(true);
    }
    try {
      ...
      const response = await fetch('https://followupio.com/swadeshordermanagment/get/inwards', {
        method: 'POST',
        headers: {
...
const renderInwardItem = ({ item }: { item: Inward }) => {
        ...
        <View style={styles.cardContent}>
          <View style={styles.cardHeader}>
            ...
            <View style={[styles.statusBadge, { backgroundColor: statusStyle.badge }]}>
              <Text style={[styles.statusBadgeText, { color: statusStyle.text }]}>{item.InwardStatus}</Text>
            </View>
          </View>
          ...
          <View style={styles.secondrow}>
            ...
            <Text style={styles.detailValue}>{formatDate(item.InwardDate)}</Text>
          </View>
  ...
  if (loading) {
    return <ActivityIndicator size="large" color="#025C42" style={{ marginTop: 20 }} />;
  }
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>No inward records found.</Text>}
        refreshControl={
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 8, marginBottom: 15, flexDirection: 'row', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  cardIndicator: { width: 6, borderTopLeftRadius: 8, borderBottomLeftRadius: 8 },
  cardContent: { flex: 1, padding: 15 },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 },
  warehouseName: { fontSize: 18, fontWeight: 'bold', color: '#333', flex: 1, marginRight: 10 },
  ...
  detailValueBold: { fontSize: 16, color: '#111', fontWeight: '600', marginLeft: 10 },
  description: { fontSize: 14, color: '#666', marginTop: 12, fontStyle: 'italic', padding: 8, borderRadius: 4 },
  footer: { borderTopWidth: 1, borderTopColor: '#f0f0f0', paddingTop: 10, marginTop: 15, alignItems: 'flex-end' },
