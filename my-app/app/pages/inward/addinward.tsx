  ...
  const fetchWarehouses = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://nodeapi.godigitalchat.com/swadeshordermanagement/get/warehouse",
        {
          method: "POST",
          headers: {
      ...
      if (data.status === 1) {
        ...
        data.message === "Invalid AuthToken" ||
        data.message === "AuthToken is required"
      ) {
  ...
  const submitInward = async () => {
    ...
    try {
      ...
      if (productsForApi.length === 0) {
        Alert.alert(
          "Validation Error",
          "Please add at least one valid item with a quantity."
        );
        setIsSaving(false);
        return;
      }
      ...
      const response = await fetch(
        "https://nodeapi.godigitalchat.com/swadeshordermanagement/add/inwards",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const data = await response.json();
      if (data.status === 1) {
        Alert.alert("Success", "Inward entry created successfully.", [
    ...
    <View style={styles.itemSummaryCard}>
      <View style={styles.itemSummaryContent}>
        ...
        <Text style={styles.itemSummaryDetails}>
          Qty: {item.InQty} {item.uom} | SKU: {item.sku}
        </Text>
      </View>
              ...
              <View style={styles.inputGroup}>
                ...
                >
                  <Text
                    style={[
                      styles.selectText,
                      !warehouseName && styles.placeholder,
                    ]}
                  >
            ...
            >
              ...
              >
                {isSaving ? (
                  ...
                  <Text style={styles.saveButtonText}>Save Inward</Text>
                )}
              </LinearGradient>
            </Pressable>
          </View>
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                <View style={styles.modalHeader}>
                  ...
                  <Pressable onPress={() => setWarehouseModalVisible(false)}>
