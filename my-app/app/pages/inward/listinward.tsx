...
export default function InwardDetailsPage() {
  ...
  const { id: inwardMasterId, warehouseName, status, userName, inwardDate, description } = params;
  ...
  const fetchInwardDetails = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('https://followupio.com/swadeshordermanagment/get/inwards_details', {
        method: 'POST',
        headers: {
      ...
      <View style={styles.productCardContent}>
        <View style={styles.productHeader}>
          ...
          <View style={styles.quantityBadge}>
            <Text style={styles.quantityBadgeText}>{`${item.InQty} ${item.UOM}`}</Text>
          </View>
        </View>
        ...
        <View style={styles.productDetailRow}>
          ...
          <Text style={styles.productDetailText}>{formatDate(item.MfdDate)}</Text>
        </View>
        ...
        <View style={styles.productDetailRow}>
         ...
         {item.Remarks ? (
          <View style={styles.productDetailRow}>
            <Ionicons name="chatbox-ellipses-outline" size={16} color="#555" />
  ...
  if (loading) {
    return <ActivityIndicator size="large" color="#025C42" style={styles.centered} />;
  }
          ...
          <>
            ...
            <View style={styles.masterCard}>
              <View style={styles.detailRow}>
                ...
                <Text style={styles.detailText}><Text style={styles.detailLabel}>Warehouse:</Text> {warehouseName}</Text>
              </View>
              
              <View style={styles.detailRow}>
                <Ionicons name="person-outline" size={20} color="#555" />
                <Text style={styles.detailText}><Text style={styles.detailLabel}>User:</Text> {userName}</Text>
              </View>
    
              <View style={styles.detailRow}>
                <Ionicons name="calendar-outline" size={20} color="#555" />
                <Text style={styles.detailText}><Text style={styles.detailLabel}>Date:</Text> {formatDate(inwardDate)}</Text>
              </View>
              
              <View style={styles.detailRow}>
                <Ionicons name="information-circle-outline" size={20} color="#555" />
                <Text style={styles.detailText}><Text style={styles.detailLabel}>Status:</Text> {status}</Text>
              </View>
    
              {description && (
                 <View style={styles.detailRow}>
                    <Ionicons name="chatbox-ellipses-outline" size={20} color="#555" />
                    <Text style={styles.detailText}><Text style={styles.detailLabel}>Remarks:</Text> {description}</Text>
                 </View>
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  // Master Card Styles
  masterCard: { backgroundColor: '#fff', borderRadius: 10, padding: 20, marginBottom: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 },
  detailRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  detailText: { fontSize: 16, color: '#444', marginLeft: 15, flex: 1 },
  detailLabel: { fontWeight: '600' },
  
  // Product Card Styles (NEW)
  productCard: { backgroundColor: '#fff', borderRadius: 8, marginBottom: 15, flexDirection: 'row', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  productCardIndicator: { width: 6, borderTopLeftRadius: 8, borderBottomLeftRadius: 8, backgroundColor: '#025C42' },
  ...
  productName: { fontSize: 15, fontWeight: 'bold', color: '#333', flex: 1, marginRight: 10 },
  quantityBadge: { backgroundColor: '#E0F2F1', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 12 },
  skuText: {fontSize: 16, fontWeight: '700', color: '#BA871F'},
  quantityBadgeText: { fontSize: 12, fontWeight: '700', color: '#00796B' },
  rowbetween: {flexDirection: 'row', justifyContent: 'space-between'},
  productDetailRow: { flexDirection: 'row', alignItems: 'center', marginTop: 8 },
  productDetailTextLabel: { fontSize: 14, color: '#666', marginLeft: 10, fontWeight: '600' },
  productDetailText: { fontSize: 14, color: '#333', marginLeft: 5, flexShrink: 1 },
});